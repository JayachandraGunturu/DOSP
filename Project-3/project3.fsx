...
sortedNodes <- Array.zeroCreate (nodes + randomNodes)
actorPool <- Array.zeroCreate (nodes + randomNodes)
keys <- Array.zeroCreate (nodes + randomNodes)
                                ...
                                if key = hashID then
                                    ...
                                    while i < m do
                                        if fingerTable.[i] = key then
                                            ...
                                            totalHops <- totalHops + h
                                            found <- true
                                            i <- m + 100 // To break loop if nearest node is found
                                            bossRef <! CheckCount count
                                        if i < m-1 then
...
let BossActor(mailbox: Actor<_>) =
    let rec loop()= actor{
        ...
        |AssignHash ->     for i in 0..nodes-1 do
                           ...
                           mailbox.Context.Self <! JoinNewNodes
        ...
        |JoinNewNodes ->    let actorID = nodes.ToString()
                            actorPool.[nodes] <- spawn system actorID  Actor

                            let mutable res = getSHA1(actorID)
                            while(dictionary.ContainsKey(int(res))) do
                                    res <- getSHA1(actorID)
                            dictionary.Add(int(res),actorPool.[nodes])

                            actorPool.[nodes] <! SetHashID (int(res))
